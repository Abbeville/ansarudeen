<template>
    <div>
        <div v-if="$page.props.flash.success && show" class="mb-8 flex items-center justify-between bg-green-500 rounded max-w-5xl mx-auto">
        ...
        <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" class="mx-auto mb-8 flex items-center justify-between bg-red-500 rounded max-w-5xl">
            <div class="flex items-center">
                ...
                <div v-else class="py-4 text-white text-sm font-medium">
                    <span v-if="Object.keys($page.props.errors).length === 1">There is one form error. {{ $page.props.errors.domains }}</span>
